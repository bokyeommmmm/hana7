...
-- ex1) 문신과 무신 모두 출력하시오.
  select title, contents from Notice
 where match(title, contents) against('문신* 무신*' IN BOOLEAN MODE);
...
explain select * from partirangetest where enteryear = 2018; -- partition 3
...
update partirangetest set enteryear=2004 where enteryear=2024;  -- 파티션도 변한다. 
...
explain select * from emptest where id = 100;
