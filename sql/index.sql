...
alter table emp add index idx_Emp_email (email); -- 의미 없음 
...
);  -- 데이터가 죄다 gamil이라 cardinality 1이다. 소용없는 격.
...
--
--
-- ex1) Emp 테이블에 mobile 이라는 컬럼 추가 varchar(11)
...
select substring(mobile,8)from emp;
alter table emp add index (
    (substring(mobile,8))
);

select * from emp where substring(mobile,8)=lpad('30',4,'0');
explain select * from emp where substring(mobile,8)=lpad('30',4,'0');
show index from emp;
-- alter table emp drop index functional_index_2;
...
alter table notice add constraint fk_Notice_writer_Emp 
...
explain select * from notice where match(title, contents) against ('조선의');
...
select title, contents from Notice
 ...
 select title, contents from Notice
 where match(title, contents) against('조선* -국왕*' IN BOOLEAN MODE);
  select title, contents from Notice
 where match(title, contents) against('조선* +문신*' IN BOOLEAN MODE);


select * from information_schema.innodb_ft_index_table;

set global innodb_ft_aux_table = 'schooldb/Notice';

OPTIMIZE TABLE schooldb.Notice;
...
-- ex1) 문신과 무신 모두 출력하시오.
  select title, contents from Notice
 where match(title, contents) against('+문신*' IN BOOLEAN MODE) OR match(title, contents) against('+무신*' IN BOOLEAN MODE);
-- ex2) 문신과 무신 중 후기 인물은 제외하시오.
select title, contents from Notice
 where match(title, contents) against('+문신* -후기*' IN BOOLEAN MODE) OR match(title, contents) against('+무신* -후기*' IN BOOLEAN MODE);
-- ex3) 문신과 무신 중 중기 인물만 출력하시오.
select title, contents from Notice
 where match(title, contents) against('+문신* +중기*' IN BOOLEAN MODE) OR match(title, contents) against('+무신* +중기*' IN BOOLEAN MODE);
