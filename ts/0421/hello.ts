...
let x: string | undefined = Math.random() > 0.5 ? "str" : undefined; //str이 문자열이어서 오류 X.

let y: string | number | undefined = x; // x가 명확한 값이어야 오류 x.
let z: string;
...
if (Array.isArray(xarr)) arr = xarr; //가능
